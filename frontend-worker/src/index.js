...
async function serveStaticFile(request, env) {
  ...
  if (pathname === '/' || pathname === '/upload') {
    return new Response(getUploadPage(env), {
      headers: {
  ...
  if (pathname === '/files') {
    return new Response(getFilesPage(env), {
      headers: {
        'Content-Type': 'text/html',
        ...corsHeaders,
      },
    ...
    <script>
        ...
        function renderListing(data, append) {
            ...
            filesList.insertAdjacentHTML('beforeend', 
                '<div id="paginationRow" class="file-item">' +
                    '<div class="file-name"></div>' +
                    '<div class="file-size"></div>' +
                    '<div class="file-date">Page ' + (currentPageIndex + 1) + '</div>' +
                    '<div class="file-actions">' +
                        '<button class="action-button" onclick="goPrevPage()"' + (canPrev ? '' : ' disabled') + '>Prev</button>' +
                        '<button class="action-button" onclick="goNextPage()"' + (canNext ? '' : ' disabled') + '>Next</button>' +
                    '</div>' +
                '</div>'
            );
        }
